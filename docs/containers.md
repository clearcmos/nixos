...
- `/etc/nixos/containers/templates/` - Source Docker Compose YAML files
...
1. Create a Docker Compose YAML file in `/etc/nixos/containers/templates/`
...
The script processes each YAML file in `/etc/nixos/containers/templates/` and generates a corresponding Nix file in `/etc/nixos/containers/`.
...
A dedicated backup system for container volumes is provided via the script `/etc/nixos/scripts/backup-containers.sh`. See [container-backups.md](./container-backups.md) for complete documentation.

### Backup Process

The backup script:
1. Gracefully stops all running containers
2. Creates a compressed archive of all container volumes
3. Restarts previously running containers
4. Manages a rolling set of backups (default: 8 maximum)

### Manual Backup

```bash
# Run a manual backup of all container volumes
...
# Extract the backup
sudo tar -xzf /mnt/syno/backups/misc/container-volumes/container-volumes-YYYYMMDD-HHMMSS.tar.gz -C /var/lib/containers/storage/volumes/

# Start all container targets
